...
[[in-proc]]
=== I would like to run Morse in my process and use to to inspect application state during development
...
:aliases
{:dev
  {:extra-deps {io.github.nubank/morse {:git/tag "v2023.04.20.01" :git/sha "73c8ed3"}}}
}
----

With this dependency in place you can run your Clojure REPL with the `:dev` alias active.
...
After starting PROC2 you can tun the following to start a remote Replicant:
...
To launch Morse in remote inspection mode run the following command:
...
By default Morse runs the version 20 of JavaFX but if you need to run Morse with different versions of the JavaFX library then you should add the following dependencies to your deps.edn file to override Morse's default. We cannot guarantee that Morse will run as expected if you change its JavaFX versions.

[source,clojure]
----
org.openjfx/javafx-fxml     {:mvn/version "JFX_VER"}
org.openjfx/javafx-controls {:mvn/version "JFX_VER"}
org.openjfx/javafx-swing    {:mvn/version "JFX_VER"}
org.openjfx/javafx-base     {:mvn/version "JFX_VER"}
org.openjfx/javafx-web      {:mvn/version "JFX_VER"}}
----

[[interface]]
== The Morse User Interface

link:#interface[image:/assets/images/content/guides/morse/morse.png["Morse UI",width=800]]
